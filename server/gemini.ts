...
export async function generateAppIdeas(): Promise<AppIdea[]> {
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    const modelToUse = attempt < MAX_RETRIES ? GEMINI_MODEL_PRO : GEMINI_MODEL_FLASH;
...
Generate 3 unique ideas NOW. Each must be completely different from common productivity tools. Focus on solving real, specific problems that people will pay to fix.`;
            ...
            properties: {
              ideas: {
                ...
                items: {
                  ...
                  required: ["name", "description", "targetAudience", "features"]
                }
              }
            },
            required: ["ideas"]
  ...
  throw new Error(`Failed to generate app ideas after ${MAX_RETRIES} attempts: ${lastError?.message}`);
}
...
3. Follow Whop's official documentation (provide these links for Replit AI to reference):
   ...
   - https://docs.whop.com/apps/app-views/experience-view
   - https://docs.whop.com/apps/building-apps
   - https://docs.whop.com/apps/tutorials
   - https://docs.whop.com/apps/get-api-key
...
Keep it simple - just describe the app requirements and reference the Whop docs. Let Replit AI figure out the technical implementation by reading the documentation.`;
  ...
  throw new Error(`Failed to generate Replit prompt after ${MAX_RETRIES} attempts: ${lastError?.message}`);
}
